...
var kafkaUrl = "172.31.88.109:29092"
...
func SaramaProducer() {
	...
	//使用配置,新建一个异步生产者
	producer, e := sarama.NewAsyncProducer([]string{kafkaUrl}, config)
	if e != nil {
		fmt.Println(e)
		return
	}
...
func SaramaConsumer() {
	...
	// config.Consumer.Offsets.CommitInterval = 1 * time.Second 这个已经弃用
	...
	client, err := sarama.NewClient([]string{kafkaUrl}, config)
	if err != nil {
	...
	partitionOffsetManager, err := offsetManager.ManagePartition("raintest", 0)
	if err != nil {
	...
	partitions, _ := consumer.Partitions("raintest")
	fmt.Println(partitions, "partitions@@@@@@@@@@@@@@@@@@")

	//第一次的offset从kafka获取(发送OffsetFetchRequest)，之后从本地获取，由MarkOffset()得来
	nextOffset, _ := partitionOffsetManager.NextOffset()
	fmt.Println(nextOffset, "partitions")

	//创建一个分区consumer，从上次提交的offset开始进行消费
	partitionConsumer, err := consumer.ConsumePartition("raintest", 0, nextOffset+1)
	if err != nil {
		panic(err)
	}
